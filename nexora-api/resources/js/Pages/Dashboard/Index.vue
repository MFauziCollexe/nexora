        ...
        <div class="p-8">
            <div class="mb-8 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
                <div>
                    ...
                    <p class="mt-2 text-sm text-slate-500">Welcome back! Here's what's happening with your API.</p>
                </div>
            </div>
            ...
            <div class="mt-8 grid grid-cols-1 gap-6 xl:grid-cols-3">
                <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm xl:col-span-2">
                    ...
                    <div class="mt-6 h-[360px] rounded-[32px] bg-slate-100 p-6">
                ...
                <div class="space-y-6">
                    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                        <div class="flex items-center justify-between">
                            <div>
                                <h2 class="text-xl font-bold text-slate-900">Top Endpoints</h2>
                                <p class="mt-1 text-sm text-slate-500">Most requested API routes this week.</p>
                            </div>
                        ...
                        <div class="mt-6 space-y-5">
                            <div v-for="item in topEndpoints" :key="item.endpoint">
                                <div class="flex items-center justify-between">
                                    <p class="font-medium text-slate-700">{{ item.endpoint }}</p>
                                    <p class="text-sm text-slate-500">{{ item.requests }}</p>
                                </div>
                    ...
                    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                        <div class="flex items-center justify-between">
                            <div>
                                <h2 class="text-xl font-bold text-slate-900">Response Status Distribution</h2>
                                <p class="mt-1 text-sm text-slate-500">Success, client, and server response share.</p>
                            </div>
                            <span class="rounded-2xl bg-slate-100 px-3 py-2 text-sm text-slate-600">88.0%</span>
                        </div>
            ...
            <div class="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div class="mt-6 overflow-x-auto">
                    <table class="min-w-full text-sm">
                        ...
                        <tbody>
                            <tr v-for="item in recentRequests" :key="item.endpoint + item.time" class="border-b border-slate-100 hover:bg-slate-50">
                                <td class="px-4 py-4">
                                    <span :class="['inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold', item.methodClass]">
                                        {{ item.method }}
                                    </span>
                                </td>
                                <td class="px-4 py-4 font-medium text-slate-700">{{ item.endpoint }}</td>
                                <td class="px-4 py-4 text-slate-600">{{ item.status }}</td>
                                <td class="px-4 py-4 text-slate-600">{{ item.response }}</td>
