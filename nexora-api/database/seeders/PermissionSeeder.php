...
{
    private array $moduleEntities = [
        'auth' => ['user', 'role', 'token', 'session'],
        'user' => ['user', 'profile'],
        'ai-chat' => ['chat', 'voice-assistant'],
        'settings' => ['setting', 'menu', 'audit-log'],
        'company' => ['branch', 'currency', 'fiscal-year'],
        'asset' => ['asset', 'asset-category', 'asset-location', 'asset-status'],
        'business-partner' => ['customer', 'supplier', 'supplier-type', 'supplier-category', 'business-partner-group'],
        'attendance' => ['attendance-log', 'shift-schedule', 'overtime', 'leave-request'],
        'human-resource' => ['employee', 'department', 'position'],
    ...
    {
        $this->seedMenuPermissions();
        $this->seedModulePermissions();
        $this->seedSuperAdminRole();
    }

    private function seedMenuPermissions(): void
    {
        $permissions = [
    ...
    private function seedModulePermissions(): void
    {
        ...
        $prefix = config('modules.permissions.prefix', 'module');
        $now = now();
        ...
        foreach ($modules as $key => $module) {
            ...
            $entities = $this->moduleEntities[$key] ?? [$key];

            foreach ($entities as $entity) {
                foreach ($actions as $action) {
                    $code = "{$prefix}.{$key}.{$entity}.{$action}";
