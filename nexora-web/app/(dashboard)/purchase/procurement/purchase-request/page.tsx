...
import { statCards, purchaseRequests, formatRp } from "@/data/purchase/purchaseRequest";

type StatusFilter = "All Status" | "Pending" | "Approved" | "Rejected" | "Draft";
...
export default function PurchaseRequestPage() {
  ...
  const departments = useMemo(
    () => Array.from(new Set(purchaseRequests.map((r) => r.department))),
    []
  );
  ...
  const filtered = useMemo(() => {
    return purchaseRequests.filter((r) => {
      ...
      const matchDept =
        filterDept === "All Department" || r.department === filterDept;

      return matchSearch && matchStatus && matchPriority && matchDept;
    });
  }, [search, filterStatus, filterPriority, filterDept]);
  ...
  return (
    <div className="p-4 space-y-3">
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6"/></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Purchase Request</span>
      </div>
      ...
      <div className="flex items-end gap-2 flex-wrap pb-1">
        <div className="flex flex-col gap-0.5">
          ...
          <select value={filterStatus} onChange={(e) => { setFilterStatus(e.target.value as StatusFilter); setCurrentPage(1); }} className={selectCls}>
            {(["All Status", "Pending", "Approved", "Rejected", "Draft"] as const).map((s) => (
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <select value={filterDept} onChange={(e) => { setFilterDept(e.target.value); setCurrentPage(1); }} className={selectCls}>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-275">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["PR No.", "Date", "Requested By", "Department", "Items", "Total Amount", "Priority", "Status", "Action"].map((h) => (
            ...
            <tbody>
              {paginated.map((r) => (
                <tr key={r.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <td className="px-4 py-3 text-[12px] text-slate-700 dark:text-slate-300 font-medium whitespace-nowrap">{r.prNo}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{r.date}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-slate-400">{r.requestedBy}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400">{r.department}</td>
        ...
        {filtered.length === 0 && (
          <div className="py-14 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No purchase requests found</p>
          </div>
        )}
        ...
        <div className="border-t border-slate-100 dark:border-slate-700 px-4 py-2.5 flex items-center gap-6 text-[12px] text-slate-600 dark:text-slate-400 bg-slate-50/50 dark:bg-slate-900/50">
          ...
          <span>Requests: <strong className="text-slate-700 dark:text-slate-300">{summary.count}</strong></span>
        </div>
