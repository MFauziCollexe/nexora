...
import { statCards, purchaseOrders, formatRp } from "@/data/purchase/purchaseOrder";

type StatusFilter = "All Status" | "Draft" | "Pending" | "Approved" | "Sent" | "Partial" | "Received" | "Cancelled";
...
export default function PurchaseOrderPage() {
  ...
  function statusClasses(status: string) {
    const map: Record<string, string> = {
      ...
      Partial: "bg-orange-100 text-orange-800 dark:bg-orange-950 dark:text-orange-400",
      Received: "bg-blue-100 text-blue-800 dark:bg-blue-950 dark:text-blue-400",
  ...
  return (
    <div className="p-4 space-y-3">
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6"/></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Purchase Order</span>
      </div>
      ...
      <div className="flex items-end gap-2 flex-wrap pb-1">
        <div className="flex flex-col gap-0.5">
          ...
          <select value={filterStatus} onChange={(e) => { setFilterStatus(e.target.value as StatusFilter); setCurrentPage(1); }} className={selectCls}>
            {(["All Status", "Draft", "Pending", "Approved", "Sent", "Partial", "Received", "Cancelled"] as const).map((s) => (
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <div className="relative">
            <input
              type="text"
              placeholder="Search PO No., title or supplier..."
              value={search}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-275">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["PO No.", "Date", "Title", "Supplier", "Items", "Total Amount", "Priority", "Status", "Action"].map((h) => (
        ...
        {filtered.length === 0 && (
          <div className="py-14 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No purchase orders found</p>
          </div>
        )}
        ...
        <div className="border-t border-slate-100 dark:border-slate-700 px-4 py-2.5 flex items-center gap-6 text-[12px] text-slate-600 dark:text-slate-400 bg-slate-50/50 dark:bg-slate-900/50">
          ...
          <span>Orders: <strong className="text-slate-700 dark:text-slate-300">{summary.count}</strong></span>
        </div>
