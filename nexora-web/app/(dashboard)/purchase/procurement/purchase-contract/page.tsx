...
import { statCards, purchaseContracts, formatRp } from "@/data/purchase/purchaseContract";

type StatusFilter = "All Status" | "Draft" | "Active" | "Expired" | "Terminated" | "Renewed";
...
export default function PurchaseContractPage() {
  ...
  const filtered = useMemo(() => {
    return purchaseContracts.filter((r) => {
      const matchSearch =
  ...
  const summary = useMemo(() => {
    const totalValue = filtered.reduce((sum, r) => sum + r.contractValue, 0);
    return { totalValue, count: filtered.length };
  }, [filtered]);
  ...
  function statusClasses(status: string) {
    const map: Record<string, string> = {
      ...
      Renewed: "bg-indigo-100 text-indigo-800 dark:bg-indigo-950 dark:text-indigo-400",
    };
  ...
  return (
    <div className="p-4 space-y-3">
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6"/></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Purchase Contract</span>
      </div>
      ...
      <div className="flex items-end gap-2 flex-wrap pb-1">
        <div className="flex flex-col gap-0.5">
          ...
          <select value={filterStatus} onChange={(e) => { setFilterStatus(e.target.value as StatusFilter); setCurrentPage(1); }} className={selectCls}>
            {(["All Status", "Draft", "Active", "Expired", "Terminated", "Renewed"] as const).map((s) => (
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <div className="relative">
            <input
              type="text"
              placeholder="Search contract no., title or supplier..."
              value={search}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-300">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["Contract No.", "Date", "Title", "Supplier", "Period", "Contract Value", "Payment", "Status", "Action"].map((h) => (
            ...
            <tbody>
              {paginated.map((r) => (
                <tr key={r.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <td className="px-4 py-3 text-[12px] text-slate-700 dark:text-slate-300 font-medium whitespace-nowrap">{r.contractNo}</td>
                  ...
                  <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-slate-400 whitespace-nowrap">{r.startDate} - {r.endDate}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-slate-400 whitespace-nowrap font-medium">{formatRp(r.contractValue)}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{r.paymentTerms}</td>
        ...
        {filtered.length === 0 && (
          <div className="py-14 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No purchase contracts found</p>
          </div>
        )}
        ...
        <div className="border-t border-slate-100 dark:border-slate-700 px-4 py-2.5 flex items-center gap-6 text-[12px] text-slate-600 dark:text-slate-400 bg-slate-50/50 dark:bg-slate-900/50">
          ...
          <span>Total Value: <strong className="text-slate-700 dark:text-slate-300">{formatRp(summary.totalValue)}</strong></span>
          <span>Contracts: <strong className="text-slate-700 dark:text-slate-300">{summary.count}</strong></span>
        </div>
