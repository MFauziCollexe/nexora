...
import { statCards, supplierPayments } from "@/data/purchase/supplierPayment";

type StatusFilter = "All Status" | "Draft" | "Submitted" | "Approved" | "Paid" | "Cancelled";
const methods = [...new Set(supplierPayments.map((p) => p.paymentMethod))];
type MethodFilter = "All Methods" | (typeof methods)[number];

export default function SupplierPaymentPage() {
  const [search, setSearch] = useState("");
  const [filterStatus, setFilterStatus] = useState<StatusFilter>("All Status");
  const [filterMethod, setFilterMethod] = useState<MethodFilter>("All Methods");
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 6;

  const filtered = useMemo(() => {
    return supplierPayments.filter((r) => {
      const matchSearch = r.paymentNo.toLowerCase().includes(search.toLowerCase()) || r.supplier.toLowerCase().includes(search.toLowerCase()) || r.referenceNumber.toLowerCase().includes(search.toLowerCase());
      const matchStatus = filterStatus === "All Status" || r.status === filterStatus;
      const matchMethod = filterMethod === "All Methods" || r.paymentMethod === filterMethod;
  ...
  const summary = useMemo(() => {
    ...
    const total = filtered.reduce((sum, r) => sum + parseRp(r.paidAmount), 0);
  ...
  return (
    <div className="p-4 space-y-3">
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <span>Payables</span>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6"/></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Supplier Payment</span>
      </div>
      ...
      <div className="flex items-end gap-2 flex-wrap pb-1">
        <div className="flex flex-col gap-0.5">
          ...
          <select value={filterStatus} onChange={(e) => { setFilterStatus(e.target.value as StatusFilter); setCurrentPage(1); }} className={selectCls}>
            {(["All Status", "Draft", "Submitted", "Approved", "Paid", "Cancelled"] as const).map((s) => (<option key={s}>{s}</option>))}
          </select>
        </div>
        <div className="flex flex-col gap-0.5">
          <label className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">Method</label>
          <select value={filterMethod} onChange={(e) => { setFilterMethod(e.target.value as MethodFilter); setCurrentPage(1); }} className={selectCls}>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-325">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["Payment No.", "Date", "Payment Date", "Supplier", "Amount", "Paid", "Method", "Ref. No.", "Status", "Action"].map((h) => (
                  <th key={h} className={`px-4 py-2.5 text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide whitespace-nowrap ${h === "Amount" || h === "Paid" ? "text-right" : h === "Action" ? "text-center" : "text-left"}`}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {paginated.map((r) => (
                <tr key={r.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <td className="px-4 py-3 text-[12px] text-slate-700 dark:text-slate-300 font-medium whitespace-nowrap">{r.paymentNo}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{r.date}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{r.paymentDate}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-slate-400">{r.supplier}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-slate-400 text-right">{r.amount}</td>
                  <td className="px-4 py-3 text-[12px] text-emerald-600 dark:text-emerald-400 text-right font-medium">{r.paidAmount}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400">{r.paymentMethod}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400">{r.referenceNumber}</td>
        ...
        {filtered.length === 0 && (
          <div className="py-14 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No supplier payments found</p>
          </div>
        )}
        ...
        <div className="border-t border-slate-100 dark:border-slate-700 px-4 py-2.5 flex items-center gap-6 text-[12px] text-slate-600 dark:text-slate-400 bg-slate-50/50 dark:bg-slate-900/50">
          ...
          <span>Payments: <strong className="text-slate-700 dark:text-slate-300">{summary.count}</strong></span>
          <span>Total Paid: <strong className="text-emerald-600 dark:text-emerald-400">Rp {summary.total.toLocaleString()}</strong></span>
        </div>
