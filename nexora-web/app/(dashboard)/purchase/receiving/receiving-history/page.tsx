...
import { statCards, receivingHistory } from "@/data/purchase/receivingHistory";

type StatusFilter = "All Status" | "Draft" | "Completed" | "Cancelled";
type TypeFilter = "All Types" | "Goods Receipt" | "Supplier Return";
...
export default function ReceivingHistoryPage() {
  ...
  const filtered = useMemo(() => {
    return receivingHistory.filter((r) => {
      const matchSearch =
      ...
      const matchType =
        filterType === "All Types" || r.documentType === filterType;
  ...
  function typeClasses(type: string) {
    const map: Record<string, string> = {
      "Goods Receipt": "bg-blue-100 text-blue-800 dark:bg-blue-950 dark:text-blue-400",
      "Supplier Return": "bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-400",
    };
    return map[type] ?? "";
  }
  ...
  return (
    <div className="p-4 space-y-3">
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <span>Receiving</span>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6"/></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Receiving History</span>
      </div>
      ...
      <div className="flex items-end gap-2 flex-wrap pb-1">
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <select value={filterType} onChange={(e) => { setFilterType(e.target.value as TypeFilter); setCurrentPage(1); }} className={selectCls}>
            {(["All Types", "Goods Receipt", "Supplier Return"] as const).map((t) => (
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <div className="relative">
            <input
              type="text"
              placeholder="Search document no, reference or supplier..."
              value={search}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-300">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["Document No.", "Date", "Type", "Reference No.", "Supplier", "Items", "Qty", "Status", "Handled By", "Action"].map((h) => (
            ...
            <tbody>
              {paginated.map((r) => (
                <tr key={r.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <td className="px-4 py-3 text-[12px] text-slate-700 dark:text-slate-300 font-medium whitespace-nowrap">{r.documentNo}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{r.date}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${typeClasses(r.documentType)}`}>
                      {r.documentType}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-slate-400 whitespace-nowrap">{r.referenceNo}</td>
                  ...
                  <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-slate-400 text-center font-medium">{r.quantity.toLocaleString()}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${statusClasses(r.status)}`}>
                      {r.status}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400">{r.handledBy}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
        ...
        {filtered.length === 0 && (
          <div className="py-14 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No receiving history found</p>
          </div>
        )}
        ...
        <div className="border-t border-slate-100 dark:border-slate-700 px-4 py-2.5 flex items-center gap-6 text-[12px] text-slate-600 dark:text-slate-400 bg-slate-50/50 dark:bg-slate-900/50">
          ...
          <span>Records: <strong className="text-slate-700 dark:text-slate-300">{summary.count}</strong></span>
        </div>
