...
import { statCards, poReports } from "@/data/purchase/poReport";

type StatusFilter = "All Status" | "Draft" | "Sent" | "Confirmed" | "Partial" | "Completed" | "Cancelled";
...
export default function PoReportPage() {
  ...
  const filtered = useMemo(() => {
    return poReports.filter((r) => {
      const matchSearch = r.poNo.toLowerCase().includes(search.toLowerCase()) || r.supplier.toLowerCase().includes(search.toLowerCase());
  ...
  const summary = useMemo(() => {
    ...
    const totalVal = filtered.reduce((sum, r) => sum + parseRp(r.totalAmount), 0);
    const totalQty = filtered.reduce((sum, r) => sum + r.totalQty, 0);
    const avgFulfill = filtered.length ? Math.round(filtered.reduce((sum, r) => sum + r.fulfilledPercent, 0) / filtered.length) : 0;
    return { totalVal, totalQty, count: filtered.length, avgFulfill };
  }, [filtered]);
  ...
  function statusClasses(status: string) {
    const map: Record<string, string> = {
      ...
      Confirmed: "bg-indigo-100 text-indigo-800 dark:bg-indigo-950 dark:text-indigo-400",
  ...
  return (
    <div className="p-4 space-y-3">
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <span>Reports</span>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6"/></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">PO Report</span>
      </div>
      ...
      <div className="flex items-end gap-2 flex-wrap pb-1">
        <div className="flex flex-col gap-0.5">
          ...
          <select value={filterStatus} onChange={(e) => { setFilterStatus(e.target.value as StatusFilter); setCurrentPage(1); }} className={selectCls}>
            {(["All Status", "Draft", "Sent", "Confirmed", "Partial", "Completed", "Cancelled"] as const).map((s) => (
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-325">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["PO No.", "Date", "Supplier", "Items", "Total Qty", "Amount", "Status", "Fulfillment", "Created By", "Action"].map((h) => (
                  <th key={h} className={`px-4 py-2.5 text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide whitespace-nowrap ${h === "Items" || h === "Total Qty" ? "text-center" : h === "Amount" ? "text-right" : "text-left"}`}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {paginated.map((r) => (
                <tr key={r.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <td className="px-4 py-3 text-[12px] text-slate-700 dark:text-slate-300 font-medium whitespace-nowrap">{r.poNo}</td>
                  ...
                  <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-slate-400 text-center font-medium">{r.totalQty.toLocaleString()}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-700 dark:text-slate-300 text-right font-medium">{r.totalAmount}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="flex-1 h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full ${barColor(r.fulfilledPercent)}`} style={{ width: `${r.fulfilledPercent}%` }} />
                      </div>
                      <span className="text-[10px] text-slate-400 w-7 text-right">{r.fulfilledPercent}%</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400">{r.createdBy}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
        ...
        {filtered.length === 0 && (
          <div className="py-14 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No PO records found</p>
          </div>
        )}
        ...
        <div className="border-t border-slate-100 dark:border-slate-700 px-4 py-2.5 flex items-center gap-6 text-[12px] text-slate-600 dark:text-slate-400 bg-slate-50/50 dark:bg-slate-900/50">
          ...
          <span>PO: <strong className="text-slate-700 dark:text-slate-300">{summary.count}</strong></span>
          <span>Total Value: <strong className="text-slate-700 dark:text-slate-300">Rp {summary.totalVal.toLocaleString()}</strong></span>
          <span>Total Qty: <strong className="text-slate-700 dark:text-slate-300">{summary.totalQty.toLocaleString()}</strong></span>
          <span>Avg Fulfillment: <strong className="text-slate-700 dark:text-slate-300">{summary.avgFulfill}%</strong></span>
        </div>
