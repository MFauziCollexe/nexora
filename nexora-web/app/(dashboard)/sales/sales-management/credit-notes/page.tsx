...
export default function CreditNotesPage() {
  ...
  const [notes, setNotes] = useState<CreditNoteItem[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.get<CreditNoteItem[]>("/api/v1/sales/credit-notes?per_page=100").then((res) => {
      setNotes(res);
      setLoading(false);
  ...
  const statCards = useMemo(() => {
    ...
    const used = notes.filter((n) => n.status === "Used").length;
    const unused = notes.filter((n) => n.status === "Unused").length;
    const partial = notes.filter((n) => n.status === "Partial").length;
    return [
      { label: "Total CN", value: total, sub: "All credit notes", icon: "file-text" as const, iconBg: "bg-slate-50 dark:bg-slate-950", iconColor: "text-slate-500" },
      { label: "Issued", value: used, sub: "Applied credit notes", icon: "check" as const, iconBg: "bg-slate-50 dark:bg-slate-950", iconColor: "text-emerald-500" },
      { label: "Unused", value: unused, sub: "Not yet applied", icon: "clock" as const, iconBg: "bg-slate-50 dark:bg-slate-950", iconColor: "text-amber-500" },
      { label: "Partial", value: partial, sub: "Partially used", icon: "send" as const, iconBg: "bg-slate-50 dark:bg-slate-950", iconColor: "text-blue-500" },
    ];
  }, [notes]);
  ...
  const filtered = useMemo(() => {
    return notes.filter((note) => {
      const matchSearch =
        note.credit_note_no.toLowerCase().includes(search.toLowerCase()) ||
        (note.invoice_no ?? "").toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-end gap-2 flex-wrap pb-1">
        ...
        <Link href="/sales/sales-management/credit-notes/new" className="inline-flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white text-[12px] font-semibold px-3 py-1.5 rounded-lg transition-colors shadow-sm">
          ...
          <table className="w-full min-w-275">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["CN No.", "Date", "Invoice No.", "Customer", "Reason", "Amount", "Used Amount", "Status", "Expires On", "Action"].map((h) => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={note.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <td className="px-4 py-3 text-[12px] text-slate-700 dark:text-slate-300 font-medium whitespace-nowrap">{note.credit_note_no}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{note.date}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{note.invoice_no ?? "-"}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-slate-400">{note.customer_name}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 max-w-36 truncate">{note.reason ?? "-"}</td>
                  ...
                  <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-slate-400 whitespace-nowrap">
                    {(note.used_amount ?? 0).toLocaleString("id-ID", { style: "currency", currency: "IDR", minimumFractionDigits: 0 })}
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${statusClasses(note.status)}`}>{note.status}</span>
                  </td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{note.expires_on ?? "-"}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <div className="flex items-center gap-1.5">
                      <Link href={`/sales/sales-management/credit-notes/${note.id}`} title="View" className="w-7 h-7 rounded-lg border border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center justify-center text-slate-400 hover:text-slate-600 transition-colors">
                      ...
                      <Link href={`/sales/sales-management/credit-notes/${note.id}/edit`} title="Edit" className="w-7 h-7 rounded-lg border border-slate-200 dark:border-slate-600 hover:bg-blue-50 dark:hover:bg-blue-950 flex items-center justify-center text-slate-400 hover:text-blue-500 transition-colors">
