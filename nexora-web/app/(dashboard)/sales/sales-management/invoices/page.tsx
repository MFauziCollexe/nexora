...
import { statCards, invoices as invoicesData } from "@/data/sales/invoices";
...
export default function InvoicesPage() {
  ...
  const customers = useMemo(() => Array.from(new Set(invoicesData.map((invoice) => invoice.customer))), []);
  ...
  const filtered = useMemo(() => {
    return invoicesData.filter((invoice) => {
      const matchSearch =
        ...
        invoice.soNo.toLowerCase().includes(search.toLowerCase());
      const matchStatus = filterStatus === "All Status" || invoice.status === filterStatus;
      const matchCustomer = filterCustomer === "All Customer" || invoice.customer === filterCustomer;
            ...
            <tbody>
              {paginated.map((invoice) => (
                <tr key={invoice.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <td className="px-4 py-3 text-[12px] text-slate-700 dark:text-slate-300 font-medium whitespace-nowrap">{invoice.invoiceNo}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{invoice.date}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-slate-400">{invoice.customer}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{invoice.soNo}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{invoice.dueDate}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-slate-400 whitespace-nowrap">{invoice.amount}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-slate-400 whitespace-nowrap">{invoice.paidAmount}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${statusClasses(invoice.status)}`}>
                      {invoice.status}
                    </span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <div className="flex items-center gap-1.5">
                      ...
                      <button title="Email" type="button" className="w-7 h-7 rounded-lg border border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center justify-center text-slate-400 hover:text-slate-600 transition-colors">
