...
import { statCards, quotations as quotationsData } from "@/data/sales/quotations";
...
export default function QuotationsPage() {
  ...
  const customers = useMemo(() => Array.from(new Set(quotationsData.map((q) => q.customer))), []);
  ...
  const filtered = useMemo(() => {
    return quotationsData.filter((q) => {
      const matchSearch =
        ...
        q.sales.toLowerCase().includes(search.toLowerCase());
      const matchStatus = filterStatus === "All Status" || q.status === filterStatus;
      const matchCustomer = filterCustomer === "All Customer" || q.customer === filterCustomer;
      ...
      <div className="flex items-end gap-2 flex-wrap pb-1">
        <div className="flex flex-col gap-0.5">
          ...
          <select value={filterStatus} onChange={(e) => { setFilterStatus(e.target.value); setCurrentPage(1); }} className={selectCls}>
            {["All Status", "Open", "Sent", "Expired", "Draft", "Converted"].map((s) => <option key={s}>{s}</option>)}
          </select>
        </div>
        ...
        <div className="flex flex-col gap-0.5">
          <label className="text-[10px] text-transparent">x</label>
          <Link
            href="/sales/sales-management/quotations/new"
            ...
            <tbody>
              {paginated.map((q) => (
                <tr key={q.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <td className="px-4 py-3 text-[12px] text-slate-700 dark:text-slate-300 font-medium whitespace-nowrap">{q.no}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{q.date}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-slate-400">{q.customer}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{q.validUntil}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-slate-400 whitespace-nowrap">{q.amount}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${statusClasses(q.status)}`}>{q.status}</span>
                  </td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{q.sales}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <div className="flex items-center gap-1.5">
                      {/* View */}
                      <button title="View" className="w-7 h-7 rounded-lg border border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center justify-center text-slate-400 hover:text-slate-600 transition-colors">
