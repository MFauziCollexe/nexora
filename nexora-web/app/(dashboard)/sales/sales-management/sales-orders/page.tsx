...
export default function SalesOrdersPage() {
  ...
  useEffect(() => {
    api.get<SalesOrderItem[]>("/api/v1/sales/sales-orders?per_page=100").then((res) => {
      setOrders(res);
      setLoading(false);
  ...
  const statCards = useMemo(() => {
    ...
    return [
      { label: "Total Sales Orders", value: total, sub: "All orders", icon: "file-text" as const, iconBg: "bg-slate-50 dark:bg-slate-950", iconColor: "text-slate-500" },
      { label: "Confirmed", value: confirmed, sub: "Ready to process", icon: "check" as const, iconBg: "bg-slate-50 dark:bg-slate-950", iconColor: "text-emerald-500" },
      { label: "Released", value: released, sub: "In warehouse", icon: "send" as const, iconBg: "bg-slate-50 dark:bg-slate-950", iconColor: "text-blue-500" },
      { label: "Pending", value: pending, sub: "Awaiting confirmation", icon: "clock" as const, iconBg: "bg-slate-50 dark:bg-slate-950", iconColor: "text-amber-500" },
    ];
  }, [orders]);
  ...
  const filtered = useMemo(() => {
    return orders.filter((order) => {
      const matchSearch =
        ...
        (order.sales_person?.name ?? "").toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div className="p-4 space-y-3">
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6"/></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Sales Orders</span>
      </div>
          ...
          <table className="w-full min-w-250">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["SO No.", "Date", "Customer", "Quotation No", "Amount", "Status", "Sales Person", "Action"].map((h) => (
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={8} className="px-4 py-14 text-center text-[12px] text-slate-400 dark:text-slate-600">Loading...</td>
                </tr>
              ) : paginated.map((order) => (
                <tr key={order.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{order.quotation_no ?? "-"}</td>
                  ...
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{order.sales_person?.name ?? "-"}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
