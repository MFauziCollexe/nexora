...
export default function AssetPage() {
  ...
  const filtered = assetData.filter((a) => {
    ...
    const matchCategory = filterCategory === "All Category" || a.category === filterCategory;
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-[1000px]">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["Asset Code", "Asset Name", "Category", "Location", "Status", "Purchase Date", "Action"].map((h) => (
            ...
            <tbody>
              {paginatedData.map((a) => (
                <tr key={a.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  ...
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{a.category}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{a.location}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`${a.statusColor} text-[11px] font-semibold px-2 py-0.5 rounded-full`}>{a.status}</span>
                  </td>
                  <td className="px-4 py-3 text-[11px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{a.purchaseDate}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No assets found</p>
          </div>
        )}
      </div>

      <DrawerForm open={drawerOpen} title="Add Asset" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">Asset Information</h3>
          <div className="space-y-3">
            {[
              { label: "Asset Code", type: "text", placeholder: "Enter asset code", required: true },
              { label: "Asset Name", type: "text", placeholder: "Enter asset name", required: true },
              { label: "Category", type: "select", placeholder: "Select category", required: true, options: ["Select category", "Kendaraan Operasional", "Genset", "Peralatan Kantor", "Peralatan IT", "Panel Listrik", "Peralatan Gudang"] },
              { label: "Location", type: "select", placeholder: "Select location", required: true, options: ["Select location", "Gudang Cold Storage A", "Gudang Cold Storage B", "Area Genset", "Ruang Meeting", "Kantor HR", "Ruang Panel", "Docking Area"] },
              { label: "Purchase Date", type: "text", placeholder: "DD MMM YYYY", required: true },
              { label: "Status", type: "select", placeholder: "Select status", required: true, options: ["Select status", "Active", "Maintenance", "Disposed"] },
        ...
        <div className="flex gap-2 pt-2 border-t border-slate-100 dark:border-slate-700">
          ...
          <button className="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-[12px] font-semibold py-2 rounded-lg transition-colors">Save Asset</button>
        </div>
      </DrawerForm>
