...
import { statCards, assetLocations as locationData } from "@/data/masterdata/assetLocation";
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6"/></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Asset Location</span>
      </div>
      ...
      <div className="flex items-end gap-2 overflow-x-auto pb-1">
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <button
            onClick={() => { setFilterStatus("All Status"); setSearch(""); setCurrentPage(1); setCurrentPage(1); }}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-[1000px]">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["Location Code", "Location Name", "Description", "Status", "Action"].map((h) => (
            ...
            <tbody>
              {paginatedData.map((l) => (
                <tr key={l.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  <td className="px-4 py-3 text-[12px] font-medium text-slate-600 dark:text-slate-300 whitespace-nowrap">{l.locationCode}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-800 dark:text-slate-200 whitespace-nowrap font-medium">{l.locationName}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400">{l.description}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`${l.statusColor} text-[11px] font-semibold px-2 py-0.5 rounded-full`}>{l.status}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No locations found</p>
          </div>
        )}
      </div>

      <DrawerForm open={drawerOpen} title="Add Location" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">Location Information</h3>
          <div className="space-y-3">
            {[
              { label: "Location Code", type: "text", placeholder: "Enter location code", required: true },
              { label: "Location Name", type: "text", placeholder: "Enter location name", required: true },
              { label: "Description", type: "textarea", placeholder: "Enter location description", required: true },
        ...
        <div className="flex gap-2 pt-2 border-t border-slate-100 dark:border-slate-700">
          ...
          <button className="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-[12px] font-semibold py-2 rounded-lg transition-colors">Save Location</button>
        </div>
      </DrawerForm>
