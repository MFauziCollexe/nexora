...
import { statCards, assetCategories as categoryData } from "@/data/masterdata/assetCategory";
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6"/></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Asset Category</span>
      </div>
          ...
          <table className="w-full min-w-[1000px]">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["Category Code", "Category Name", "Description", "Status", "Action"].map((h) => (
            ...
            <tbody>
              {paginatedData.map((c) => (
                <tr key={c.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  <td className="px-4 py-3 text-[12px] font-medium text-slate-600 dark:text-slate-300 whitespace-nowrap">{c.categoryCode}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-800 dark:text-slate-200 whitespace-nowrap font-medium">{c.categoryName}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400">{c.description}</td>
      ...
      <DrawerForm open={drawerOpen} title="Add Category" onClose={() => setDrawerOpen(false)}>
        <div>
          ...
          <div className="space-y-3">
            {[
              ...
              { label: "Description", type: "textarea", placeholder: "Enter category description", required: true },
        ...
        <div className="flex gap-2 pt-2 border-t border-slate-100 dark:border-slate-700">
          ...
          <button className="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-[12px] font-semibold py-2 rounded-lg transition-colors">Save Category</button>
        </div>
      </DrawerForm>
