...
import { statCards, positions as positionData } from "@/data/masterdata/position";
...
export default function PositionPage() {
  ...
  const filtered = positionData.filter((p) => {
    const matchSearch =
      p.code.toLowerCase().includes(search.toLowerCase()) ||
      p.name.toLowerCase().includes(search.toLowerCase()) ||
      p.description.toLowerCase().includes(search.toLowerCase());
    const matchDepartment = filterDepartment === "All Departments" || p.department === filterDepartment;
    const matchLevel = filterLevel === "All Levels" || p.level === filterLevel;
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6"/></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Position</span>
      </div>
      ...
      <div className="flex items-end gap-2 overflow-x-auto pb-1">
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <select value={filterLevel} onChange={(e) => { setFilterLevel(e.target.value); setCurrentPage(1); }} className={inputClass}>
            {['All Levels', 'Executive', 'Manager', 'Supervisor', 'Staff'].map((l) => <option key={l}>{l}</option>)}
          </select>
        </div>
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <button
            onClick={() => { setFilterDepartment('All Departments'); setFilterLevel('All Levels'); setFilterStatus('All Status'); setSearch(''); setCurrentPage(1); setCurrentPage(1); }}
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <div className="relative">
            <input
              type="text" placeholder="Search position..."
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[950px]">
            ...
            <tbody>
              {paginatedData.map((p) => (
                <tr key={p.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-slate-300 whitespace-nowrap">{p.code}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-800 dark:text-slate-200 whitespace-nowrap">{p.name}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{p.department}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{p.level}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400">{p.description}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`${p.statusColor} text-[11px] font-semibold px-2 py-0.5 rounded-full`}>{p.status}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No positions found</p>
          </div>
        )}
      </div>

      <DrawerForm open={drawerOpen} title="Add New Position" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">Position Details</h3>
          <div className="space-y-3">
            {[
              { label: "Position Code", type: "text", placeholder: "Enter position code", required: true },
              { label: "Position Name", type: "text", placeholder: "Enter position name", required: true },
              { label: "Department", type: "select", placeholder: "Select department", required: true, options: ["Select department", "Management", "Human Resource", "Finance", "Information Technology", "Marketing", "Sales", "Production"] },
              { label: "Level", type: "select", placeholder: "Select level", required: true, options: ["Select level", "Executive", "Manager", "Supervisor", "Staff"] },
