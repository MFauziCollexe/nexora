...
import { statCards, employees as employeeData } from "@/data/masterdata/employee";
...
export default function EmployeePage() {
  ...
  const filtered = employeeData.filter((e) => {
    ...
    const matchDepartment = filterDepartment === "All Department" || e.department === filterDepartment;
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6"/></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Employee</span>
      </div>
      ...
      <div className="flex items-end gap-2 overflow-x-auto pb-1">
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <select value={filterDepartment} onChange={(e) => { setFilterDepartment(e.target.value); setCurrentPage(1); }} className={inputClass}>
            {["All Department", "Management", "Finance", "Accounting", "Human Resource", "Information Technology", "Marketing", "Sales", "Tax"].map((d) => <option key={d}>{d}</option>)}
          </select>
        </div>
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <button
            onClick={() => { setFilterStatus("All Status"); setFilterDepartment("All Department"); setSearch(""); setCurrentPage(1); setCurrentPage(1); }}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-[1000px]">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["Employee ID", "Full Name", "Department", "Position", "Email", "Phone", "Status", "Join Date", "Action"].map((h) => (
            ...
            <tbody>
              {paginatedData.map((e) => (
                <tr key={e.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">

                  <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-slate-300 whitespace-nowrap">{e.employeeId}</td>

                  <td className="px-4 py-3 whitespace-nowrap">
                    <div className="flex items-center gap-2.5">
                      <div className="w-7 h-7 rounded-full bg-gradient-to-br from-slate-400 to-slate-600 flex items-center justify-center text-white text-[10px] font-bold flex-shrink-0">{e.avatar}</div>
                      <span className="text-[12px] font-medium text-slate-700 dark:text-slate-300">{e.fullName}</span>
                    </div>
                  </td>

                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{e.department}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{e.position}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{e.email}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{e.phone}</td>

                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`${e.statusColor} text-[11px] font-semibold px-2 py-0.5 rounded-full`}>{e.status}</span>
                  </td>

                  <td className="px-4 py-3 text-[11px] text-slate-500 dark:text-slate-400 whitespace-pre-line">{e.joinDate}</td>
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No employees found</p>
          </div>
        )}
      </div>

      <DrawerForm open={drawerOpen} title="Add Employee" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">Employee Details</h3>
          <div className="space-y-3">
            {[
              { label: "Employee ID", type: "text", placeholder: "Enter employee id", required: true },
              { label: "Full Name", type: "text", placeholder: "Enter full name", required: true },
              { label: "Department", type: "select", placeholder: "Select department", required: true, options: ["Select department", "Management", "Finance", "Accounting", "Human Resource", "Information Technology", "Marketing", "Sales"] },
              { label: "Position", type: "text", placeholder: "Enter position", required: true },
              { label: "Email", type: "email", placeholder: "Enter email address", required: true },
              { label: "Phone", type: "text", placeholder: "Enter phone number", required: false },
              { label: "Join Date", type: "text", placeholder: "DD MMM YYYY", required: false },
              ...
              <div key={field.label}>
                ...
                {field.type === "select" ? (
                  ...
                  field.type === "textarea" ? (
                    <textarea rows={4} placeholder={field.placeholder} className="w-full border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 rounded-lg px-3 py-2 text-[12px] text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900" />
                  ) : (
                    <input type={field.type} placeholder={field.placeholder} className="w-full border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 rounded-lg px-3 py-2 text-[12px] text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900" />
                  )
                )}
              </div>
            ))}
        ...
        <div className="flex gap-2 pt-2 border-t border-slate-100 dark:border-slate-700">
          ...
          <button className="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-[12px] font-semibold py-2 rounded-lg transition-colors">Save Employee</button>
        </div>
      </DrawerForm>
