...
import { statCards, paymentTerms } from "@/data/masterdata/paymentTerms";
...
export default function PaymentTermsPage() {
  ...
  const filtered = paymentTerms.filter((item) => {
    const matchSearch =
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <span>Finance</span>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6"/></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Payment Terms</span>
      </div>
      ...
      <div className="flex items-end gap-2 overflow-x-auto pb-1">
        <div className="flex flex-col gap-0.5">
          ...
          <select value={filterType} onChange={(e) => { setFilterType(e.target.value); setCurrentPage(1); }} className={inputClass}>
            {['All Type', 'Cash', 'Credit', 'Others'].map((type) => <option key={type}>{type}</option>)}
          </select>
        </div>
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <button
            onClick={() => { setFilterType('All Type'); setFilterStatus('All Status'); setSearch(''); setCurrentPage(1); setCurrentPage(1); }}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-[1000px]">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {['Code', 'Payment Term', 'Type', 'Description', 'Due Days', 'Status', 'Action'].map((header) => (
              ...
              {paginatedData.map((item) => (
                <tr key={item.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`${item.typeColor} text-[11px] font-semibold px-2 py-0.5 rounded-full`}>{item.type}</span>
                  </td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400">{item.description}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{item.dueDays}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No payment terms found</p>
          </div>
        )}
      </div>

      <DrawerForm open={drawerOpen} title="Add Payment Term" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">Payment Term Details</h3>
          <div className="space-y-3">
            {[
              { label: "Code", type: "text", placeholder: "Enter term code", required: true },
              { label: "Payment Term", type: "text", placeholder: "Enter payment term name", required: true },
              { label: "Type", type: "select", placeholder: "Select type", required: true, options: ["Cash", "Credit", "Others"] },
              { label: "Description", type: "text", placeholder: "Enter description", required: false },
              { label: "Due Days", type: "number", placeholder: "Enter due days", required: true },
