...
import { statCards, bankAccounts as bankAccountData } from "@/data/masterdata/bankAccount";
...
export default function BankAccountPage() {
  ...
  const filtered = bankAccountData.filter((c) => {
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <span>Finance</span>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6" /></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Bank Account</span>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-250">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["Code", "Bank Name", "Account Name", "Account Number", "Currency", "Status", "Action"].map((h) => (
            ...
            <tbody>
              {paginatedData.map((c) => (
                <tr key={c.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  ...
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-800 dark:text-slate-200">{c.bankName}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-800 dark:text-slate-200">{c.accountName}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] font-mono text-slate-700 dark:text-slate-300">{c.accountNumber}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-blue-50 text-blue-600 dark:bg-blue-950 dark:text-blue-400">{c.currency}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No bank accounts found</p>
          </div>
        )}
      </div>

      <DrawerForm open={drawerOpen} title="Add Bank Account" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">Bank Account Information</h3>
          <div className="space-y-3">
            {[
              { label: "Code", type: "text", placeholder: "e.g. BA-013", required: true },
              { label: "Bank Name", type: "text", placeholder: "e.g. Bank Mandiri", required: true },
              { label: "Account Name", type: "text", placeholder: "Enter account name", required: true },
              { label: "Account Number", type: "text", placeholder: "Enter account number", required: true },
              { label: "Currency", type: "select", placeholder: "Select currency", required: true, options: ["IDR", "USD", "EUR", "SGD"] },
