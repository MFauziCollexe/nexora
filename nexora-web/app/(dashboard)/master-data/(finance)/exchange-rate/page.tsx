...
import { statCards, exchangeRates as exchangeRateData } from "@/data/masterdata/exchangeRate";
...
export default function ExchangeRatePage() {
  ...
  const filtered = exchangeRateData.filter((c) => {
    const query = search.toLowerCase();
    const matchSearch =
      c.fromCurrency.toLowerCase().includes(query) ||
      c.toCurrency.toLowerCase().includes(query);
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <span>Finance</span>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6" /></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Exchange Rate</span>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-250">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["#", "From", "To", "Rate", "Effective Date", "Status", "Action"].map((h) => (
            ...
            <tbody>
              {paginatedData.map((c) => (
                <tr key={c.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] font-medium text-slate-700 dark:text-slate-300">{c.id}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-blue-50 text-blue-600 dark:bg-blue-950 dark:text-blue-400">{c.fromCurrency}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-600 dark:bg-emerald-950 dark:text-emerald-400">{c.toCurrency}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] font-mono font-medium text-slate-800 dark:text-slate-200">
                    {c.rate.toLocaleString("id-ID", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{c.effectiveDate}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`${c.statusColor} text-[11px] font-semibold px-2 py-0.5 rounded-full`}>{c.status}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No exchange rates found</p>
          </div>
        )}
      </div>

      <DrawerForm open={drawerOpen} title="Add Exchange Rate" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">Exchange Rate Information</h3>
          <div className="space-y-3">
            {[
              { label: "From Currency", type: "select", placeholder: "Select from currency", required: true, options: ["USD", "EUR", "GBP", "SGD", "JPY", "AUD", "CNY", "MYR"] },
              { label: "To Currency", type: "select", placeholder: "Select to currency", required: true, options: ["IDR", "USD", "EUR", "SGD"] },
              { label: "Rate", type: "number", placeholder: "e.g. 16250.00", required: true },
              { label: "Effective Date", type: "date", placeholder: "Select date", required: true },
