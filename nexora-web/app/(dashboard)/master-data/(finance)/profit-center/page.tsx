...
import { statCards, profitCenters as profitCenterData } from "@/data/masterdata/profitCenter";
...
export default function ProfitCenterPage() {
  ...
  const filtered = profitCenterData.filter((c) => {
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <span>Finance</span>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6" /></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Profit Center</span>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-250">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["Code", "Profit Center Name", "Description", "Status", "Last Updated", "Action"].map((h) => (
            ...
            <tbody>
              {paginatedData.map((c) => (
                <tr key={c.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  ...
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 max-w-xs truncate">{c.description}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`${c.statusColor} text-[11px] font-semibold px-2 py-0.5 rounded-full`}>{c.status}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{c.updatedAt}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No profit centers found</p>
          </div>
        )}
      </div>

      <DrawerForm open={drawerOpen} title="Add Profit Center" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">Profit Center Information</h3>
          <div className="space-y-3">
            {[
              { label: "Code", type: "text", placeholder: "e.g. PC-013", required: true },
              { label: "Name", type: "text", placeholder: "Enter profit center name", required: true },
