...
import { regions, statCards, countries as countryData } from "@/data/masterdata/country";

export default function CountryPage() {
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [search, setSearch] = useState("");
  const [filterRegion, setFilterRegion] = useState("All Region");
  ...
  const filtered = countryData.filter((country) => {
    ...
    const matchRegion = filterRegion === "All Region" || country.region === filterRegion;
    const matchStatus = filterStatus === "All Status" || country.status === filterStatus;
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-end gap-2 overflow-x-auto pb-1">
        ...
        <div className="flex flex-col gap-0.5">
          <label className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">Region</label>
          <select value={filterRegion} onChange={(e) => { setFilterRegion(e.target.value); setCurrentPage(1); }} className={inputClass}>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-250">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {['Code', 'Country Name', 'Country Code (ISO 2)', 'Region', 'Status', 'Action'].map((h) => (
            ...
            <tbody>
              {paginatedData.map((country) => (
                <tr key={country.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-600 dark:text-slate-300">{country.code}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-800 dark:text-slate-200">{country.countryName}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{country.countryCode3}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{country.region}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`${country.statusColor} text-[11px] font-semibold px-2 py-0.5 rounded-full`}>{country.status}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No countries found</p>
          </div>
        )}
      </div>

      <DrawerForm open={drawerOpen} title="Add Country" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">General Information</h3>
          <div className="space-y-3">
            {[
              { label: "Country Code (ISO 2)", type: "text", placeholder: "Enter 2 letter code (ID, SG, US)", required: true },
              { label: "Country Name", type: "text", placeholder: "Enter country name", required: true },
              { label: "Country Code (ISO 3)", type: "text", placeholder: "Enter 3 letter code (ISO 3) optional", required: false },
              { label: "Region", type: "select", placeholder: "Select region", required: true, options: regions.filter((region) => region !== "All Region") },
