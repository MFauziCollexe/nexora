...
import { baseCurrencyOptions, currencies, countries, statCards } from "@/data/masterdata/currency";

export default function CurrencyPage() {
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [search, setSearch] = useState("");
  const [filterStatus, setFilterStatus] = useState("All Status");
  const [filterBaseCurrency, setFilterBaseCurrency] = useState("All Base Currency");
  ...
  const filtered = currencies.filter((currency) => {
    ...
    const matchStatus = filterStatus === "All Status" || currency.status === filterStatus;
    const matchBaseCurrency = filterBaseCurrency === "All Base Currency" || currency.baseCurrency === filterBaseCurrency;
    const matchCountry = filterCountry === "All Country" || currency.country === filterCountry;
    return matchSearch && matchStatus && matchBaseCurrency && matchCountry;
  });
  ...
  return (
    <div className="p-4 space-y-3">
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <span>General</span>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6"/></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Currency</span>
      </div>
      ...
      <div className="flex items-end gap-2 overflow-x-auto pb-1">
        ...
        <div className="flex flex-col gap-0.5">
          <label className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">Base Currency</label>
          <select value={filterBaseCurrency} onChange={(e) => { setFilterBaseCurrency(e.target.value); setCurrentPage(1); }} className={inputClass}>
            {baseCurrencyOptions.map((option) => <option key={option}>{option}</option>)}
          </select>
        </div>
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <button
            onClick={() => {
              ...
              setFilterBaseCurrency("All Base Currency");
              setFilterCountry("All Country");
              setSearch("");
              setCurrentPage(1);
            }}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-[980px]">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {['Code', 'Currency Name', 'Symbol', 'Country', 'Decimal Places', 'Base Currency', 'Status', 'Action'].map((h) => (
            ...
            <tbody>
              {paginatedData.map((currency) => (
                <tr key={currency.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-600 dark:text-slate-300">{currency.code}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-800 dark:text-slate-200">{currency.currencyName}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{currency.symbol}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{currency.country}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{currency.decimalPlaces}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{currency.baseCurrency}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`${currency.statusColor} text-[11px] font-semibold px-2 py-0.5 rounded-full`}>{currency.status}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No currencies found</p>
          </div>
        )}
      </div>

      <DrawerForm open={drawerOpen} title="Add Currency" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">General Information</h3>
          <div className="space-y-3">
            {[
              { label: "Currency Code (ISO 3)", type: "text", placeholder: "Enter 3 letter code (IDR, USD, EUR)", required: true },
              { label: "Currency Name", type: "text", placeholder: "Enter currency name", required: true },
              { label: "Symbol", type: "text", placeholder: "Enter currency symbol (Rp, $, €, ¥)", required: true },
              { label: "Country", type: "select", placeholder: "Select country", required: true, options: countries.filter((country) => country !== "All Country") },
              { label: "Decimal Places", type: "select", placeholder: "Select decimal places", required: true, options: ["Select decimal places", "0", "1", "2", "3"] },
              { label: "Base Currency", type: "select", placeholder: "Base currency?", required: true, options: ["Select base currency", "Yes", "No"] },
