...
import { provinces, statCards, cities as cityData } from "@/data/masterdata/city";

export default function CityPage() {
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [search, setSearch] = useState("");
  const [filterProvince, setFilterProvince] = useState("All Province");
  ...
  const filtered = cityData.filter((city) => {
    ...
    const matchSearch =
      ...
      city.postalCode.toLowerCase().includes(query);
    const matchProvince = filterProvince === "All Province" || city.province === filterProvince;
    const matchStatus = filterStatus === "All Status" || city.status === filterStatus;
    ...
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-end gap-2 overflow-x-auto pb-1">
        ...
        <div className="flex flex-col gap-0.5">
          <label className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">Province</label>
          <select value={filterProvince} onChange={(e) => { setFilterProvince(e.target.value); setCurrentPage(1); }} className={inputClass}>
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <button
            onClick={() => { setFilterProvince("All Province"); setFilterStatus("All Status"); setSearch(""); setCurrentPage(1); }}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-250">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {['City Code', 'City Name', 'Province', 'Postal Code', 'Status', 'Action'].map((h) => (
            ...
            <tbody>
              {paginatedData.map((city) => (
                <tr key={city.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-600 dark:text-slate-300">{city.code}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-800 dark:text-slate-200">{city.cityName}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{city.province}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{city.postalCode}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`${city.statusColor} text-[11px] font-semibold px-2 py-0.5 rounded-full`}>{city.status}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No cities found</p>
          </div>
        )}
      </div>

      <DrawerForm open={drawerOpen} title="Add City" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">General Information</h3>
          <div className="space-y-3">
            {[
              { label: "City Code", type: "text", placeholder: "Enter city code", required: true },
              { label: "City Name", type: "text", placeholder: "Enter city name", required: true },
              { label: "Province", type: "select", placeholder: "Select province", required: true, options: provinces.filter((prov) => prov !== "All Province") },
              { label: "Postal Code", type: "text", placeholder: "Enter postal code (optional)", required: false },
