...
import { statCards, itemGroups as itemGroupData } from "@/data/masterdata/itemGroup";
...
export default function ItemGroupPage() {
  ...
  const filtered = itemGroupData.filter((g) => {
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-250">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["Group Code", "Group Name", "Description", "Status", "Action"].map((h) => (
            ...
            <tbody>
              {paginatedData.map((g) => (
                <tr key={g.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-600 dark:text-slate-300">{g.code}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-800 dark:text-slate-200">{g.name}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{g.description}</td>
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No item groups found</p>
          </div>
        )}
      </div>

      <DrawerForm open={drawerOpen} title="Add Item Group" onClose={() => setDrawerOpen(false)}>
        <div>
