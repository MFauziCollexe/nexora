...
import { statCards, reorderPoints as reorderPointData, categories } from "@/data/masterdata/reorderPoint";
...
export default function ReorderPointPage() {
  ...
  const filtered = reorderPointData.filter((r) => {
    const query = search.toLowerCase();
    const matchSearch =
      r.code.toLowerCase().includes(query) ||
      r.name.toLowerCase().includes(query) ||
      r.category.toLowerCase().includes(query);
    const matchCategory = filterCategory === "All Category" || r.category === filterCategory;
    const matchStock = filterStock === "All Stock Status" || r.stockStatus === filterStock;
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <span>Inventory</span>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6" /></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Reorder Point</span>
      </div>
      ...
      <div className="flex items-end gap-2 overflow-x-auto pb-1">
        ...
        <div className="flex flex-col gap-0.5">
          <label className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">Stock Status</label>
          <select value={filterStock} onChange={(e) => { setFilterStock(e.target.value); setCurrentPage(1); }} className={inputClass}>
            {["All Stock Status", "Adequate", "Need Reorder", "Low Stock", "Overstock"].map((s) => <option key={s}>{s}</option>)}
          </select>
        </div>

        <div className="flex flex-col gap-0.5">
          <label className="text-[10px] text-transparent">x</label>
          <button onClick={() => { setFilterCategory("All Category"); setFilterStock("All Stock Status"); setSearch(""); setCurrentPage(1); }} className={btnOutline}>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-250">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["Item Code", "Item Name", "Category", "Current Stock", "Min Stock", "Max Stock", "Reorder Point", "Stock Status", "Action"].map((h) => (
            ...
            <tbody>
              {paginatedData.map((r) => (
                <tr key={r.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] font-medium text-slate-700 dark:text-slate-300">{r.code}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-800 dark:text-slate-200">
                    <div className="flex items-center gap-2.5">
                      <div className="w-7 h-7 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-slate-700 dark:text-slate-200 text-[10px] font-semibold">{r.image}</div>
                      <span>{r.name}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-600 dark:text-slate-400">{r.category}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-700 dark:text-slate-300 font-medium">{r.currentStock.toLocaleString("id-ID")}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{r.minStock.toLocaleString("id-ID")}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{r.maxStock.toLocaleString("id-ID")}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className="bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-400 text-[12px] font-semibold px-2 py-0.5 rounded-full">{r.reorderPoint.toLocaleString("id-ID")}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`${r.stockStatusColor} text-[11px] font-semibold px-2 py-0.5 rounded-full`}>{r.stockStatus}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No reorder points found</p>
          </div>
        )}
      </div>

      <DrawerForm open={drawerOpen} title="Update Reorder Point" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">Reorder Point Information</h3>
          <div className="space-y-3">
            {[
              { label: "Item", type: "select", placeholder: "Select item", required: true, options: reorderPointData.map((r) => `${r.code} - ${r.name}`) },
              ...
              { label: "Max Stock", type: "text", placeholder: "Enter maximum stock", required: true },
              { label: "Reorder Point", type: "text", placeholder: "Enter reorder point", required: true },
            ].map((field) => (
              <div key={field.label}>
