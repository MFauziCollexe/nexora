...
import { statCards, batchLots as batchLotData, itemNames } from "@/data/masterdata/batchLotMaster";
...
export default function BatchLotMasterPage() {
  ...
  const filtered = batchLotData.filter((b) => {
    const query = search.toLowerCase();
    const matchSearch =
      b.batchNumber.toLowerCase().includes(query) ||
      b.item.toLowerCase().includes(query) ||
      b.supplier.toLowerCase().includes(query) ||
      b.notes.toLowerCase().includes(query);
    const matchItem = filterItem === "All Item" || b.item === filterItem;
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <span>Inventory</span>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6" /></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Batch/Lot Master</span>
      </div>
      ...
      <div className="flex items-end gap-2 overflow-x-auto pb-1">
        ...
        <div className="flex flex-col gap-0.5">
          <label className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">Item</label>
          <select value={filterItem} onChange={(e) => { setFilterItem(e.target.value); setCurrentPage(1); }} className={inputClass}>
            {itemNames.map((item) => <option key={item}>{item}</option>)}
          </select>
        </div>
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <select value={filterStatus} onChange={(e) => { setFilterStatus(e.target.value); setCurrentPage(1); }} className={inputClass}>
            {["All Status", "Active", "Expiring Soon", "Expired"].map((s) => <option key={s}>{s}</option>)}
          </select>
        </div>

        <div className="flex flex-col gap-0.5">
          <label className="text-[10px] text-transparent">x</label>
          <button onClick={() => { setFilterItem("All Item"); setFilterStatus("All Status"); setSearch(""); setCurrentPage(1); }} className={btnOutline}>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-250">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["Batch Number", "Item", "Quantity", "Manufacture Date", "Expiry Date", "Supplier", "Notes", "Status", "Action"].map((h) => (
            ...
            <tbody>
              {paginatedData.map((b) => (
                <tr key={b.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] font-medium text-slate-700 dark:text-slate-300">{b.batchNumber}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-800 dark:text-slate-200">{b.item}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-600 dark:text-slate-300">{b.quantity}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{b.manufactureDate}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{b.expiryDate ?? "-"}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-600 dark:text-slate-400">{b.supplier}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 max-w-xs truncate">{b.notes}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`${b.statusColor} text-[11px] font-semibold px-2 py-0.5 rounded-full`}>{b.status}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No batch/lot records found</p>
          </div>
        )}
      </div>

      <DrawerForm open={drawerOpen} title="Add Batch/Lot" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">Batch/Lot Information</h3>
          <div className="space-y-3">
            {[
              { label: "Batch Number", type: "text", placeholder: "Enter batch number", required: true },
              { label: "Item", type: "select", placeholder: "Select item", required: true, options: itemNames.filter((i) => i !== "All Item") },
              { label: "Quantity", type: "text", placeholder: "Enter quantity", required: true },
              { label: "Manufacture Date", type: "text", placeholder: "Enter manufacture date (YYYY-MM-DD)", required: true },
              { label: "Expiry Date", type: "text", placeholder: "Enter expiry date (YYYY-MM-DD)", required: false },
              { label: "Supplier", type: "text", placeholder: "Enter supplier name", required: false },
              { label: "Notes", type: "textarea", placeholder: "Enter notes", required: false },
              { label: "Status", type: "select", placeholder: "Select status", required: true, options: ["Select status", "Active", "Expiring Soon", "Expired"] },
            ].map((field) => (
              <div key={field.label}>
