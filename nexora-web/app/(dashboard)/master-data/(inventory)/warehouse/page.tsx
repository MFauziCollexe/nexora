...
import { statCards, warehouses as warehouseData } from "@/data/masterdata/warehouse";
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <span>Inventory</span>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6"/></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Warehouse</span>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-250">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {['Code', 'Warehouse Name', 'Description', 'Location', 'Status', 'Action'].map((h) => (
            ...
            <tbody>
              {paginatedData.map((warehouse) => (
                <tr key={warehouse.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-slate-300 whitespace-nowrap">{warehouse.code}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-800 dark:text-slate-200 whitespace-nowrap">{warehouse.name}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{warehouse.description}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{warehouse.location}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`${warehouse.statusColor} text-[11px] font-semibold px-2 py-0.5 rounded-full`}>{warehouse.status}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No warehouses found</p>
          </div>
        )}
      </div>

      <DrawerForm open={drawerOpen} title="Add Warehouse" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">Warehouse Information</h3>
          <div className="space-y-3">
            {[
              { label: "Warehouse Code", type: "text", placeholder: "Enter warehouse code", required: true },
              { label: "Warehouse Name", type: "text", placeholder: "Enter warehouse name", required: true },
