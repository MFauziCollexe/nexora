...
import { statCards, binLocations as binLocationData, warehouses } from "@/data/masterdata/binLocation";
...
export default function BinLocationPage() {
  ...
  const filtered = binLocationData.filter((b) => {
    const query = search.toLowerCase();
    const matchSearch =
      b.code.toLowerCase().includes(query) ||
      b.name.toLowerCase().includes(query) ||
      b.warehouse.toLowerCase().includes(query);
    const matchWarehouse = filterWarehouse === "All Warehouse" || b.warehouse === filterWarehouse;
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <span>Inventory</span>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6" /></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Bin Location</span>
      </div>
      ...
      <div className="flex items-end gap-2 overflow-x-auto pb-1">
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <button onClick={() => { setFilterWarehouse("All Warehouse"); setFilterStatus("All Status"); setSearch(""); setCurrentPage(1); }} className={btnOutline}>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-250">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["Bin Code", "Bin Name", "Warehouse", "Aisle", "Rack", "Shelf", "Bin", "Max Capacity", "Status", "Action"].map((h) => (
            ...
            <tbody>
              {paginatedData.map((b) => (
                <tr key={b.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] font-medium text-slate-700 dark:text-slate-300">{b.code}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-800 dark:text-slate-200">{b.name}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-600 dark:text-slate-400">{b.warehouse}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{b.aisle}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{b.rack}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{b.shelf}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{b.bin}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-600 dark:text-slate-300">{b.maxCapacity} {b.capacityUom}</td>
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No bin locations found</p>
          </div>
        )}
      </div>

      <DrawerForm open={drawerOpen} title="Add Bin Location" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">Location Information</h3>
          <div className="space-y-3">
            {[
              { label: "Bin Code", type: "text", placeholder: "Enter bin code", required: true },
              { label: "Bin Name", type: "text", placeholder: "Enter bin name", required: true },
              { label: "Warehouse", type: "select", placeholder: "Select warehouse", required: true, options: warehouses.filter((w) => w !== "All Warehouse") },
              { label: "Aisle", type: "text", placeholder: "Enter aisle (e.g. A, B, C)", required: false },
              { label: "Rack", type: "text", placeholder: "Enter rack (e.g. R01, R02)", required: false },
              { label: "Shelf", type: "text", placeholder: "Enter shelf (e.g. S1, S2)", required: false },
              { label: "Bin", type: "text", placeholder: "Enter bin (e.g. B01, P01)", required: false },
              { label: "Max Capacity", type: "text", placeholder: "Enter max capacity", required: false },
              { label: "Capacity UOM", type: "select", placeholder: "Select UOM", required: false, options: ["KG", "PCS", "Liter", "Meter", "UNIT"] },
