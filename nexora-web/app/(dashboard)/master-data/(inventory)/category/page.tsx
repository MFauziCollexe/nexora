...
import { statCards, categories as categoryData } from "@/data/masterdata/category";
...
export default function CategoryPage() {
  ...
  const filtered = categoryData.filter((category) => {
    ...
    const matchSearch =
      ...
      category.parent.toLowerCase().includes(query);
    const matchStatus = filterStatus === "All Status" || category.status === filterStatus;
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-250">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["Category Code", "Category Name", "Description", "Parent Category", "Status", "Action"].map((h) => (
            ...
            <tbody>
              {paginatedData.map((category) => (
                <tr key={category.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-600 dark:text-slate-300">{category.code}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-800 dark:text-slate-200">{category.name}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{category.description}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{category.parent}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`${category.statusColor} text-[11px] font-semibold px-2 py-0.5 rounded-full`}>{category.status}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No categories found</p>
          </div>
        )}
      </div>

      <DrawerForm open={drawerOpen} title="Add Category" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">Category Information</h3>
          <div className="space-y-3">
            {[
              { label: "Category Code", type: "text", placeholder: "Enter category code", required: true },
              { label: "Category Name", type: "text", placeholder: "Enter category name", required: true },
              { label: "Description", type: "textarea", placeholder: "Enter description", required: false },
              { label: "Parent Category", type: "select", placeholder: "Select parent category", required: false, options: ["-", ...categoryData.map((category) => category.name)] },
              ...
              <div key={field.label}>
                ...
                {field.type === "select" ? (
                  <select className="w-full border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 rounded-lg px-3 py-2 text-[12px] text-slate-500 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900">
                    {field.options?.map((option, index) => <option key={`${field.label}-${index}`} value={option}>{option}</option>)}
                  </select>
                ) : field.type === "textarea" ? (
                  <textarea
                    rows={4}
