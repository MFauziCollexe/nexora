...
import { statCards, serialNumbers as serialNumberData, itemNames, statuses } from "@/data/masterdata/serialNumberMaster";

export default function SerialNumberMasterPage() {
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [search, setSearch] = useState("");
  const [filterItem, setFilterItem] = useState("All Item");
  const [filterStatus, setFilterStatus] = useState("All Status");

  const filtered = serialNumberData.filter((s) => {
    const query = search.toLowerCase();
    const matchSearch =
      s.serialNumber.toLowerCase().includes(query) ||
      s.item.toLowerCase().includes(query) ||
      s.batchLot.toLowerCase().includes(query) ||
      s.notes.toLowerCase().includes(query);
    const matchItem = filterItem === "All Item" || s.item === filterItem;
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <span>Inventory</span>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6" /></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Serial Number Master</span>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-250">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["Serial Number", "Item", "Batch/Lot", "Status", "Warehouse", "Purchase Date", "Warranty Expiry", "Notes", "Action"].map((h) => (
            ...
            <tbody>
              {paginatedData.map((s) => (
                <tr key={s.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] font-medium text-slate-700 dark:text-slate-300">{s.serialNumber}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-800 dark:text-slate-200">{s.item}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-600 dark:text-slate-400">{s.batchLot}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`${s.statusColor} text-[11px] font-semibold px-2 py-0.5 rounded-full`}>{s.status}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-600 dark:text-slate-400">{s.warehouse}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{s.purchaseDate}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{s.warrantyExpiry ?? "-"}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 max-w-xs truncate">{s.notes}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No serial numbers found</p>
          </div>
        )}
      </div>

      <DrawerForm open={drawerOpen} title="Add Serial Number" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">Serial Number Information</h3>
          <div className="space-y-3">
            {[
              { label: "Serial Number", type: "text", placeholder: "Enter serial number", required: true },
              { label: "Item", type: "select", placeholder: "Select item", required: true, options: itemNames.filter((i) => i !== "All Item") },
              { label: "Batch/Lot", type: "select", placeholder: "Select batch/lot", required: false, options: ["-", "BATCH-2024-001", "BATCH-2024-002", "BATCH-2024-003", "BATCH-2024-005", "BATCH-2023-001"] },
              { label: "Status", type: "select", placeholder: "Select status", required: true, options: ["Select status", "Available", "Sold", "Used", "Scrap", "In Repair", "Returned"] },
              { label: "Warehouse", type: "text", placeholder: "Enter warehouse location", required: false },
              { label: "Purchase Date", type: "text", placeholder: "Enter purchase date (YYYY-MM-DD)", required: false },
              { label: "Warranty Expiry", type: "text", placeholder: "Enter warranty expiry (YYYY-MM-DD)", required: false },
