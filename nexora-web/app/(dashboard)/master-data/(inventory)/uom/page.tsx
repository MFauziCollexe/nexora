...
import { statCards, uoms as uomData } from "@/data/masterdata/uom";
...
export default function UomPage() {
  ...
  const filtered = uomData.filter((uom) => {
    const query = search.toLowerCase();
    const matchSearch =
      uom.code.toLowerCase().includes(query) ||
      uom.name.toLowerCase().includes(query) ||
      uom.type.toLowerCase().includes(query) ||
      uom.base.toLowerCase().includes(query) ||
      uom.conversion.toLowerCase().includes(query) ||
      uom.description.toLowerCase().includes(query);
    const matchStatus = filterStatus === "All Status" || uom.status === filterStatus;
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <span>Inventory</span>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6"/></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">UOM</span>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-250">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["UOM Code", "UOM Name", "UOM Type", "Base UOM", "Conversion to Base", "Description", "Status", "Action"].map((h) => (
            ...
            <tbody>
              {paginatedData.map((uom) => (
                <tr key={uom.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-slate-300 whitespace-nowrap">{uom.code}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-800 dark:text-slate-200 whitespace-nowrap">{uom.name}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{uom.type}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{uom.base}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{uom.conversion}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-500 dark:text-slate-400 whitespace-nowrap">{uom.description}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`${uom.statusColor} text-[11px] font-semibold px-2 py-0.5 rounded-full`}>{uom.status}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No UOM found</p>
          </div>
        )}
      </div>

      <DrawerForm open={drawerOpen} title="Add UOM" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">UOM Details</h3>
          <div className="space-y-3">
            {[
              { label: "UOM Code", type: "text", placeholder: "Enter UOM code", required: true },
              { label: "UOM Name", type: "text", placeholder: "Enter UOM name", required: true },
              { label: "UOM Type", type: "select", placeholder: "Select UOM type", required: true, options: ["Select type", "Base Unit", "Non-Base Unit"] },
              { label: "Base UOM", type: "text", placeholder: "Enter base UOM", required: true },
              { label: "Conversion to Base", type: "text", placeholder: "Enter conversion formula", required: false },
