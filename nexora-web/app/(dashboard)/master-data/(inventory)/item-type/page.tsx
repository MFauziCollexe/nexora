...
import { statCards, itemTypes as itemTypeData } from "@/data/masterdata/itemType";
...
export default function ItemTypePage() {
  ...
  const filtered = itemTypeData.filter((t) => {
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-250">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["Type Code", "Type Name", "Description", "Status", "Action"].map((h) => (
            ...
            <tbody>
              {paginatedData.map((t) => (
                <tr key={t.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-600 dark:text-slate-300">{t.code}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-800 dark:text-slate-200">{t.name}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{t.description}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`${t.statusColor} text-[11px] font-semibold px-2 py-0.5 rounded-full`}>{t.status}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No item types found</p>
          </div>
        )}
      </div>

      <DrawerForm open={drawerOpen} title="Add Item Type" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">Type Information</h3>
          <div className="space-y-3">
            {[
              { label: "Type Code", type: "text", placeholder: "Enter type code", required: true },
              { label: "Type Name", type: "text", placeholder: "Enter type name", required: true },
