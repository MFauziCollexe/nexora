...
import { statCards, branches as branchData } from "@/data/masterdata/branch";
...
export default function BranchPage() {
  ...
  const filtered = branchData.filter((c) => {
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6" /></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Branch</span>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-250">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["Code", "Branch Name", "City", "Phone", "Head Office", "Status", "Action"].map((h) => (
            ...
            <tbody>
              {paginatedData.map((c) => (
                <tr key={c.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] font-medium text-slate-700 dark:text-slate-300">{c.code}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-800 dark:text-slate-200">{c.name}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-500 dark:text-slate-400">{c.city}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] font-mono text-slate-600 dark:text-slate-400">{c.phone}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    {c.isHeadOffice ? (
                      <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-amber-50 text-amber-600 dark:bg-amber-950 dark:text-amber-400">Head Office</span>
                    ) : (
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No branches found</p>
          </div>
        )}
      </div>

      <DrawerForm open={drawerOpen} title="Add Branch" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">Branch Information</h3>
          <div className="space-y-3">
            {[
              { label: "Code", type: "text", placeholder: "e.g. BR-009", required: true },
              { label: "Branch Name", type: "text", placeholder: "Enter branch name", required: true },
              { label: "Address", type: "textarea", placeholder: "Enter address", required: false },
              { label: "City", type: "text", placeholder: "Enter city", required: false },
              { label: "Phone", type: "text", placeholder: "e.g. 021-12345678", required: false },
              { label: "Email", type: "text", placeholder: "Enter email", required: false },
              { label: "Head Office", type: "select", placeholder: "Select type", required: true, options: ["Yes", "No"] },
