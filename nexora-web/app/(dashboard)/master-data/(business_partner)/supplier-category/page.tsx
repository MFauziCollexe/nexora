...
import { statCards, supplierCategories as supplierCategoryData } from "@/data/masterdata/supplierCategory";
...
export default function SupplierCategoryPage() {
  ...
  const filtered = supplierCategoryData.filter((st) => {
    const query = search.toLowerCase();
    const matchSearch =
      st.code.toLowerCase().includes(query) ||
      st.name.toLowerCase().includes(query) ||
      st.description.toLowerCase().includes(query);
    const matchStatus = filterStatus === "All Status" || st.status === filterStatus;
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6" /></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Supplier Category</span>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-250">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["Supplier Category Code", "Supplier Category Name", "Description", "Status", "Actions"].map((h) => (
            ...
            <tbody>
              {paginatedData.map((st) => (
                <tr key={st.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] font-medium text-slate-700 dark:text-slate-300">{st.code}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-800 dark:text-slate-200">{st.name}</td>
                  <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-slate-400 max-w-xs truncate">{st.description}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`${st.statusColor} text-[11px] font-semibold px-2 py-0.5 rounded-full`}>{st.status}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No supplier categories found</p>
          </div>
        )}
      ...
      <DrawerForm open={drawerOpen} title="Add Supplier Category" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">Supplier Category Information</h3>
          <div className="space-y-3">
            {[
              { label: "Supplier Category Code", type: "text", placeholder: "Enter supplier category code", required: true },
              { label: "Supplier Category Name", type: "text", placeholder: "Enter supplier category name", required: true },
