...
import { statCards, businessPartnerGroups as businessPartnerGroupData, groupTypes } from "@/data/masterdata/businessPartnerGroup";

export default function BusinessPartnerGroupPage() {
  ...
  const filtered = businessPartnerGroupData.filter((g) => {
    const query = search.toLowerCase();
    const matchSearch =
      g.code.toLowerCase().includes(query) ||
      g.name.toLowerCase().includes(query) ||
      g.description.toLowerCase().includes(query);
    const matchType = filterType === "All Types" || g.type === filterType;
    const matchStatus = filterStatus === "All Status" || g.status === filterStatus;
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6" /></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Business Partner Group</span>
      </div>
      ...
      <div className="flex items-end gap-2 overflow-x-auto pb-1">
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <button onClick={() => { setFilterType("All Types"); setFilterStatus("All Status"); setSearch(""); setCurrentPage(1); }} className={btnOutline}>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-250">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["Group Code", "Group Name", "Type", "Description", "Status", "Actions"].map((h) => (
            ...
            <tbody>
              {paginatedData.map((g) => (
                <tr key={g.id} className="border-b border-slate-50 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] font-medium text-slate-700 dark:text-slate-300">{g.code}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-[12px] text-slate-800 dark:text-slate-200">{g.name}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`${g.type === "Both" ? "bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400" : g.type === "Customer" ? "bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400" : "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400"} text-[11px] font-semibold px-2 py-0.5 rounded-full`}>{g.type}</span>
                  </td>
                  <td className="px-4 py-3 text-[12px] text-slate-600 dark:text-slate-400 max-w-xs truncate">{g.description}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className={`${g.statusColor} text-[11px] font-semibold px-2 py-0.5 rounded-full`}>{g.status}</span>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No business partner groups found</p>
          </div>
        )}
      ...
      <DrawerForm open={drawerOpen} title="Add Business Partner Group" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">Group Information</h3>
          <div className="space-y-3">
            {[
              { label: "Group Code", type: "text", placeholder: "Enter group code", required: true },
              { label: "Group Name", type: "text", placeholder: "Enter group name", required: true },
              { label: "Type", type: "select", placeholder: "Select type", required: true, options: ["Select type", "Customer", "Supplier", "Both"] },
