...
import { statCards, supplierTypes as supplierTypeData } from "@/data/masterdata/supplierType";
...
export default function SupplierTypePage() {
  ...
  const filtered = supplierTypeData.filter((st) => {
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500">
        ...
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M9 18l6-6-6-6" /></svg>
        <span className="text-slate-600 dark:text-slate-300 font-medium">Supplier Type</span>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-250">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
                {["Supplier Type Code", "Supplier Type Name", "Description", "Status", "Actions"].map((h) => (
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-[12px] text-slate-400 dark:text-slate-600">No supplier types found</p>
          </div>
        )}
      ...
      <DrawerForm open={drawerOpen} title="Add Supplier Type" onClose={() => setDrawerOpen(false)}>
        <div>
          <h3 className="text-[12px] font-bold text-slate-700 dark:text-slate-300 mb-3">Supplier Type Information</h3>
          <div className="space-y-3">
            {[
              { label: "Supplier Type Code", type: "text", placeholder: "Enter supplier type code", required: true },
              { label: "Supplier Type Name", type: "text", placeholder: "Enter supplier type name", required: true },
